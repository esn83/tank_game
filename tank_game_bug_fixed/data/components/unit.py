...
class Unit:
    ...
    def update(self, dt):
        ...
        if not self.is_dead:
            ...
            if self.attack_target != None:
                ...
                # print('atan2',target_angle)

                # as_polar
                # works
                target_angle = self.limit_angle_to_360((self.pos - self.attack_target).as_polar()[1])
                target_angle_int = int(target_angle)
                ...
                if abs(angle_diff) < angle_step:
                    ...
                    if abs(angle_diff) > angle_step:
                        self.top_angle_prev = self.top_angle_current - angle_diff
                elif cw:
                    self.top_angle_current -= angle_step
                    if self.top_angle_current < 0:
                        self.top_angle_current = 359
                        self.top_angle_prev = 359
                else:
                    self.top_angle_current += angle_step
            ...
            move_step = self.move_speed/100 * dt

            if self.path != None and len(self.path) > 0:

                node_rect_center = pg.Vector2(pg.Rect(self.path[0].world_rect).center)

                # as_polar
                # works
                target_angle = self.limit_angle_to_360((self.pos - node_rect_center).as_polar()[1])
                target_angle_int = int(target_angle)
                ...
                # decide base rotation direction clockwise/counter clockwise
                cw = True
                angle_diff = self.base_angle_current-target_angle_to_image_angle
                angle_diff_360 = self.limit_angle_to_360(angle_diff)
                if angle_diff_360 > 180:
                    cw = False

                angle_step = int(self.base_angle_speed/100 * dt)
                if abs(angle_diff) < angle_step:
                    self.base_angle_current = target_angle_to_image_angle
                    if abs(angle_diff) > angle_step:
                        self.base_angle_prev = self.base_angle_current - angle_diff
                elif cw:
                    self.base_angle_current -= angle_step
                    if self.base_angle_current < 0:
                        self.base_angle_current = 359
                        self.base_angle_prev = 359
                else:
                    self.base_angle_current += angle_step
            ...
            if self.is_moving_left:
                self.pos += pg.Vector2(-1,0) * move_step
            if self.is_moving_right:
                self.pos += pg.Vector2(1,0) * move_step
            if self.is_moving_up:
                self.pos += pg.Vector2(0,-1) * move_step
    ...
    def set_base_angle(self, angle):
        ...
        self.base_angle_prev = angle
        self.update_rotate_interpolate_and_mask(1)

    def set_top_angle(self, angle):
        self.top_angle_current = angle
        self.top_angle_prev = angle
        self.update_rotate_interpolate_and_mask(1)
